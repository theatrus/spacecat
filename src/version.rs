...
/// the plain `chatstronomy` name.
pub const WORDMARK: &str = "Chatstronomy";

/// Crate version from Cargo.toml.
pub const VERSION: &str = env!("CARGO_PKG_VERSION");

/// Git SHA the binary was built from (`unknown` when built without git
/// metadata, e.g. from a source tarball without `CHATSTRONOMY_GIT_SHA` set).
pub const GIT_SHA: &str = env!("CHATSTRONOMY_GIT_SHA");

/// `<version> (<sha>)`, e.g. `0.3.0 (1a2b3c4d5e6f)`.
pub const VERSION_STRING: &str = concat!(
    env!("CARGO_PKG_VERSION"),
    " (",
    env!("CHATSTRONOMY_GIT_SHA"),
    ")"
);
