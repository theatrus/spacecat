...
impl ServiceWrapper {
    ...
    pub fn run_with_shutdown(
        ...
        rt.block_on(async {
            let client = SpaceCatApiClient::new(self.config.api.clone()).map_err(|e| {
            ...
            self.run_service_loop(poller, poll_interval, shutdown_rx).await
        })
    }
    ...
    #[cfg(all(windows, feature = "windows-service"))]
    async fn initialize_baseline(&self, poller: &mut DiscordUpdater) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    #[cfg(all(windows, feature = "windows-service"))]
    async fn poll_once(&self, poller: &mut DiscordUpdater) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
