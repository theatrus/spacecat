...
cp config.example.json ~/.config/chatstronomy/config.json
```

### Configuration

Chatstronomy uses a single `config.json` describing **shared chat infrastructure**
...
```json
{
  "logging": { "level": "info", "enable_file_logging": false, "log_file": "chatstronomy.log" },
  "chat": {
    "discord": {
      "enabled": true,
...
chatstronomy --telescope c925 sequence

# Event history
chatstronomy events
chatstronomy last-events --count 20
...
chatstronomy chat-updater --interval 5
```
...
chatstronomy --config /path/to/custom-config.json sequence
chatstronomy --config ~/observatory/config.json --telescope c925 mount-info
```
...
- **`/chatstronomy` slash commands** — read-only inspection (`status`, `sequence`, `target`,
...
- **Channel-driven defaults** — `/chatstronomy status` invoked in a telescope's channel
...
- **Live status embed** *(opt-in via `chat.discord_bot.live_status`)* — one pinned status
  ...
  filter changes, mount events, etc.). Message IDs are persisted in `chatstronomy-state.json`
...
5. Slash command global registration takes up to an hour on a new bot. After that, type `/chatstronomy`
...
"chat": {
  "matrix": {
    ...
    "username": "@chatstronomy:matrix.org",
    "password": "your_password",
...
Chatstronomy auto-joins any pending room invitations on startup.
...
| `state_file` | string | Where to persist live-status message IDs (default `./chatstronomy-state.json`) |
...
| `username` | string | e.g. `@chatstronomy:matrix.org` |
...
Chatstronomy validates the config on startup. Common errors:
...
Chatstronomy talks to a number of NINA Advanced API endpoints. All are under `/v2/api/`:
...
Write endpoints used by the `/chatstronomy ...` Discord slash commands:

| Endpoint | Triggered by |
|---|---|
| `/equipment/mount/park` | `/chatstronomy park` |
| `/equipment/mount/unpark` | `/chatstronomy unpark` |
| `/equipment/mount/home` | `/chatstronomy home` |
| `/equipment/filterwheel/change-filter?filterId=N` | `/chatstronomy change-filter <name>` |
| `/equipment/focuser/auto-focus?cancel=<bool>` | `/chatstronomy autofocus [cancel:true]` |
| `/equipment/guider/start?calibrate=<bool>` | `/chatstronomy guider-start` |
| `/equipment/guider/stop` | `/chatstronomy guider-stop` |
| `/equipment/camera/cool?temperature=T&minutes=M` | `/chatstronomy cool <temp>` |
| `/equipment/camera/warm?minutes=M` | `/chatstronomy warm` |
| `/equipment/camera/abort-exposure` | `/chatstronomy abort-capture` |
| `/sequence/start?skipValidation=<bool>` | `/chatstronomy start-sequence` |
| `/sequence/stop` | `/chatstronomy stop-sequence` |
...
├── api.rs               # ChatstronomyApiClient — read + write helpers
├── chat/
│   ├── mod.rs           # ChatService trait + ChatServiceManager + ChatTarget
│   ├── discord_service.rs  # Webhook-based posting
│   ├── discord_bot.rs   # Serenity/Poise bot with /chatstronomy commands
